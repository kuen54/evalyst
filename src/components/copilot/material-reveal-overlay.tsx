 ...
 * 返回：wave 中心到达卡片中心的时刻，夹在 [0, 1250] 区间。
 *
...
export function computeRevealDelay(centerXvw: number): number {
  ...
  return Math.max(0, Math.min(1250, raw))
}
...
export function MaterialRevealOverlay() {
  ...
  useLayoutEffect(() => {
    ...
    const cleanupDelay = prefersReduce ? 220 : 1600

    /**
     * 清理：重新 querySelectorAll（不用前面捕获的集合），因为 1600ms 内 DOM 可能变化。
    ...
    // lastOpenedAt 再次变化（< 1600ms 内二次打开）→ 先清再重起
    return () => {
      clearTimeout(timer)
      cleanup()
    }
