...
| Glass UI 7 档 | `docs/conventions/glass-ui.md` `## 7 档梯度` 段；4 primitive (`thin` / `regular` / `thick` / `tinted`) + 3 semantic (`success` / `warning` / `danger`)；典型：`thin`=数据密集行级卡 / `regular`=页面主外壳默认 / `thick`=浮层；组件实现 `src/copilot/components/shell.tsx`。Sticky 顶/底结构条已 inline 到 `sticky-chrome.tsx`，不算 7 档玻璃成员，用 `<GlassStickyHeader>` / `<GlassStickyFooter>` |
